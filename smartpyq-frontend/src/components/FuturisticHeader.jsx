...
const FuturisticHeader = memo(() => {
  ...
  useEffect(() => { setShowUserMenu(false); setShowMobileMenu(false); }, [location.pathname, location.search]);
  const handleLogout = () => { logout(); setShowUserMenu(false); navigate('/login'); };
    ...
    <header className={'fh ' + (entered ? 'fh--entered' : '')} role='banner'>
      ...
      {isAuthenticated && isDemoUser && (
        <div style={{
          ...
          zIndex: 45, padding: '4px 16px', borderRadius: '0 0 8px 8px',
          background: 'linear-gradient(135deg, rgba(168,85,247,0.3), rgba(59,130,246,0.3))',
          backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.15)',
          borderBottom: 'none', fontSize: '11px', color: 'rgba(255,255,255,0.8)',
          fontWeight: 500, letterSpacing: '0.5px', whiteSpace: 'nowrap'
        }}>
