...
async def send_otp_simple(request: SendOTPRequest):
    ...
    await auth_svc.send_otp(email=request.email, purpose=request.purpose)
    return MessageResponse(message=f"OTP sent to {request.email}. Check server console for dev mode.")
