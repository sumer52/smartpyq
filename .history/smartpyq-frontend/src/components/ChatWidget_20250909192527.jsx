...
const ChatWidget = ({ className = "" }) => {
  ...
  const sseUrl = `${import.meta.env.VITE_BACKEND_URL || 'http://192.168.31.237:8000'}/api/v1/chat/stream?session_id=${sessionId}`;
  ...
  const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'http://192.168.31.237:8000'}/api/v1/chat`, {
        method: 'POST',
        headers: {
