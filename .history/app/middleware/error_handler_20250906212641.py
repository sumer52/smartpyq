...
# Middleware function for use with FastAPI
async def error_handler_middleware(request: Request, call_next: Callable) -> Response:
    """Error handler middleware function."""
    middleware = ErrorHandlerMiddleware(None)
    return await middleware._handle_exception(request, Exception())


# Exception handlers for specific HTTP status codes
...
async def internal_server_error_handler(request: Request, exc):
    ...
    logger.error(
        f"Internal server error in {request.method} {request.url.path}: {str(exc)}"
    )
