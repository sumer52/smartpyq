...
class ChatService:
    ...
    async def create_session(
        ...
        session = await self.chat_repo.create_session(**session_data)
        
        # Log audit event
        await self._log_audit(
            AuditAction.CHAT_SESSION_CREATE,
            actor_id=user.id,
    ...
    async def get_session(
        ...
        """
        session = await self.chat_repo.get_session_by_uuid(session_uuid)
        if not session:
    ...
    async def chat(
        ...
        if chat_request.session_id:
            try:
                ...
                session = await self.chat_repo.get_session_by_uuid(chat_request.session_id)
        ...
        if not session:
            ...
            session = await self.chat_repo.get_session_by_uuid(session_response.session_uuid)
        ...
        try:
            ...
            # Log error
            await self._log_audit(
                AuditAction.CHAT_ERROR,
                actor_id=user.id,
                target_type="chat_session",
                target_id=session.id,
                tenant_id=user.tenant_id,
                details=f"Chat error: {str(e)}",
                ip_address=ip_address,
    ...
    async def stream_chat(
        ...
        try:
            ...
            await self._log_audit(
                ...
                target_id=session.id,
                tenant_id=user.tenant_id,
                details=f"Streaming chat error: {str(e)}",
                ip_address=ip_address,
                severity=AuditSeverity.ERROR
            )
    ...
    async def get_chat_history(
        ...
        # Get messages
        messages, total = await self.message_repo.get_session_messages(
            session.id,
            page=page,
            limit=limit
        )
    ...
    async def delete_session(
        ...
        await self.chat_repo.delete_session(session.id)
        
        # Log audit event
        await self._log_audit(
            AuditAction.CHAT_SESSION_DELETE,
            actor_id=user.id,
            target_type="chat_session",
            target_id=session.id,
            tenant_id=user.tenant_id,
            details=f"Chat session deleted: {session_uuid}",
            ip_address=ip_address
        )
    ...
    async def _validate_message_content(self, content: str) -> None:
        ...
        if self.content_filter:
            if not await self.content_filter.is_safe(content):
                raise ValidationError("Message content violates safety guidelines")
