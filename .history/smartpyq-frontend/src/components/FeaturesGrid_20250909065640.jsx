...
const FeaturesGrid = () => {
  // TEST: Add a visible message to confirm rendering
  const testMessage = (
    <div style={{ background: '#ff0000', color: '#fff', padding: '16px', textAlign: 'center', fontWeight: 'bold', fontSize: '20px', zIndex: 9999 }}>
      TEST: FeaturesGrid component is rendering!
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          {features.map((feature) => (
            <motion.div
              key={feature.id}
              className={`group relative bg-white rounded-2xl border-4 ${idx === 0 ? 'border-red-600 bg-yellow-100' : feature.borderColor} ${feature.hoverBorderColor} ${feature.bgHover} transition-all duration-300 cursor-pointer overflow-hidden`}
