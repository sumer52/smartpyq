...
async def start_analysis(
    ...
    try:
        ...
        for pid in paper_ids:
            ...
            if paper:
                fp = paper.file_url
                if fp and fp.startswith("/files/"):
                    fp = os.path.join(settings.LOCAL_STORAGE_PATH, fp[len("/files/"):].lstrip("/"))
                if fp and os.path.exists(fp):
                    extracted = extract_questions_from_pdf(fp)
                    for eq in extracted:
                        q = Question(paper_id=paper.id, question_number=eq.question_number,
                        ...
                        db.add(q)
                    # Flush once per paper instead of committing; single commit at the end
                    await db.flush()
        ...
        for row in dups:
            ...
            db.add(g)
            await db.flush()
            qr = await db.execute(select(Question).filter(Question.normalized_question_text == str(norm_text)))
        ...
        ar.status = AnalysisStatus.FAILED; ar.error_message = str(e); await db.commit()
    await db.refresh(ar)
